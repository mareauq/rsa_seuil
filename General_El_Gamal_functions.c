...
/* Réduction modulo P = X^1279 + X^319 + 1 */
void fq_reduce(mpz_t a) {
    for (long i = mpz_sizeinbase(a, 2) - 1; i >= DEG_P; i--) {
        if (mpz_tstbit(a, i)) {
            mpz_clrbit(a, i);
            mpz_setbit(a, i - DEG_P);          // +1
            mpz_setbit(a, i - DEG_P + DEG_P2); // +X^319
        }
    }
}

/* Multiplication dans Fq */
void fq_mul(mpz_t r, const mpz_t a, const mpz_t b) {
    mpz_t tmp;
    mpz_init(tmp);
    mpz_set_ui(tmp, 0);

    for (long i = 0; i < mpz_sizeinbase(b, 2); i++) {
        if (mpz_tstbit(b, i)) {
            mpz_t shifted;
            mpz_init(shifted);
...
/* Mise au carré (optimisable, mais version simple) */
void fq_square(mpz_t r, const mpz_t a) {
    fq_mul(r, a, a);
}

/* Exponentiation a^e dans Fq */
void fq_pow(mpz_t r, const mpz_t a, const mpz_t e) {
    mpz_t result, base;
    mpz_init_set_ui(result, 1);
    mpz_init_set(base, a);

    for (long i = 0; i < mpz_sizeinbase(e, 2); i++) {
        if (mpz_tstbit(e, i)) {
            fq_mul(result, result, base);
        }
        fq_square(base, base);
    }
...
/* Calcule le degré de a */
long fq_deg(const mpz_t a)     
{
    if (mpz_cmp_ui(a, 0) == 0) return -1;
    return mpz_sizeinbase(a, 2) - 1;
}

/* Inversion dans Fq : r = a^{-1} */
int fq_inv(mpz_t r, const mpz_t a) {
    mpz_t u, v, g1, g2, tmp;
    mpz_init_set(u, a);
    mpz_init(v);
