...
void KeccakF1600_StatePermute(void *state)
{
    ...
    for(round=0; round<24; round++) {
        {   /* === Î¸ step (see [Keccak Reference, Section 2.3.2]) === */
            tKeccakLane C[5], D;
            ...
            for(x=0; x<5; x++) {
                /* Compute the Î¸ effect for a given column */
                D = C[(x+4)%5] ^ ROL64(C[(x+1)%5], 1);
                /* Add the Î¸ effect to the whole column */
                for (y=0; y<5; y++)
                    XORLane(x, y, D);
            }
        }

        {   /* === Ï and Ï€ steps (see [Keccak Reference, Sections 2.3.3 and 2.3.4]) === */
            tKeccakLane current, temp;
            /* Start at coordinates (1 0) */
            x = 1; y = 0;
            current = readLane(x, y);
            /* Iterate over ((0 1)(2 3))^t * (1 0) for 0 â‰¤ t â‰¤ 23 */
            for(t=0; t<24; t++) {
        ...
        {   /* === Ï‡ step (see [Keccak Reference, Section 2.3.1]) === */
            tKeccakLane temp[5];
            for(y=0; y<5; y++) {
                /* Take a copy of the plane */
                for(x=0; x<5; x++)
                    temp[x] = readLane(x, y);
                /* Compute Ï‡ on the plane */
                for(x=0; x<5; x++)
                    writeLane(x, y, temp[x] ^((~temp[(x+1)%5]) & temp[(x+2)%5]));
            }
        }

        {   /* === Î¹ step (see [Keccak Reference, Section 2.3.5]) === */
            for(j=0; j<7; j++) {
